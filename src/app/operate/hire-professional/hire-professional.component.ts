...
export class HireProfessionalComponent implements OnInit {
  ...
  constructor(private user: NodeService,public router: Router , private route: ActivatedRoute, 
    public home:HomeNodeService, private fb: FormBuilder,) { }
  ...
  ngOnInit() {
    ...
    this.home.getProfessionalList().subscribe(
      data => {
  ...
  changeState($event) {
   ...
   navigateProfDetail(value) {
    this.router.navigate(['/professional-detail'], { queryParams: { prof_id: value } });
  }

   sendMail(){
    this.submitted = true;
    if (this.contactProfessionalData.invalid) { return; }
    else{
  ...
  get f() { return this.contactProfessionalData.controls; };
  ...
  hire(value){
    ...
    if(this.actionType == 'add'){
        this.user.hireProfessional(post).subscribe( result =>{
          var data =result;
          if(data>0){
            alert("request Send");       
          }
      ...
      this.user.updateProfessional(post).subscribe( result =>{
        var data =result;
        if(data>0){
          alert("request Send");       
        }
    ...
    this.user.buyerPrincipleAgent(buyerData).subscribe( result =>{
      var data =result;
      if(data>0){
        alert("request Send");       
      }
  ...
  if(this.actionType == 'add'){
      this.user.hireBizProf(details).subscribe( result =>{
        var data =result;
        if(data>0){
          alert("request Send");       
        }
    ...
    this.user.updateBizProf(details).subscribe( result =>{
      var data =result;
      if(data>0){
        alert("request Send");       
      }
  ...
  this.user.sellerPrincipleAgent(post_data).subscribe( result =>{
    var data =result;
    if(data>0){
      alert("request Send");       
    }
